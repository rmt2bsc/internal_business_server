...
/**
 * Test the idenity and invocation of the user login related JMS messages for
...
public class UserLoginJmsTest extends BaseMockSingleConsumerMDBTest {
    ...
    public void invokeHandelrSuccess_Login() {
        String request = RMT2File.getFileContentsAsString("xml/authentication/userauth/UserLoginRequest.xml");
        ...
        try {
            this.startTest();
            Mockito.verify(this.mockApi).authenticate(isA(String.class), isA(String.class));
        } catch (Exception e) {
            e.printStackTrace();
